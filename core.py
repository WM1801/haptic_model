from potential_profile import PiecewiseProfile
from force_profile import ForceProfile
...
class HapticSimulation:
    ...
    def __init__(self, x_min=-100.0, x_max=100.0, mass=1.0, damping=0.1):
        ...
        self.potential_profile = PiecewiseProfile()
    ...
    def set_profile(self, profile):
        ...
        self.potential_profile = profile

    def get_current_force(self):
        # Теперь возвращает только силу профиля
        return self.potential_profile.force(self.state.x)

    def get_user_feel_force(self):
        # Сила, которую ощущает пользователь: профиль + трение
        F_profile = self.potential_profile.force(self.state.x)
    ...
    def _impedance_step(self):
        ...
        F_potential = self.potential_profile.force(self.state.x)
        F_arbitrary = self.force_profile.force(self.state.x)
        F_haptic = F_potential + F_arbitrary
